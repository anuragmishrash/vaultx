function calcFutureValue(amount, annualRatePercent, years) {
  return Math.round(amount * Math.pow(1 + annualRatePercent / 100, years));
}

function shouldShowFutureValue(amount, years = 5, rate = 12) {
  const fv = calcFutureValue(amount, rate, years);
  const gain = fv - amount;
  return gain >= 50; 
}

function getFutureValueLabel(amount, years = 5, rate = 12) {
